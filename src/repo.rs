...
/// Create initial bare repo from Overleaf snapshot
fn initial_create(
    cfg: &Config,
    project_id: &str,
    source_dir: &Path,
    ...
    // add & commit
    run_git(&["add", "-A"], tmp)?;
    let msg = format!("Initial snapshot from Overleaf project {project_id}");
...
/// Sync changes from Overleaf data dir into existing bare repo
fn sync_existing(
    cfg: &Config,
    project_id: &str,
    source_dir: &Path,
    ...
    // mirror Overleaf project files into tmp working tree
