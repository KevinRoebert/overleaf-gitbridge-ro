...
async fn main() {
    ...
    info!("starting overleaf-gitbridge-ro on port {}", cfg.port);
...
fn init_storage(cfg: &Config) -> Result<(), String> {
    ...
    if !cfg.overleaf_data_path.exists() {
        warn!(path = %cfg.overleaf_data_path.display(), "overleaf data path does not exist (yet)");
    } else if !cfg.overleaf_data_path.is_dir() {
        warn!(path = %cfg.overleaf_data_path.display(), "overleaf data path is not a directory");
    }

    let projects_root = cfg.overleaf_data_path.join(&cfg.projects_dir);
...
async fn health() -> impl IntoResponse {
    (StatusCode::OK, "overleaf-gitbridge-ro up\n")
}
